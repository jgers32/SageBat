...
# Parameters matching the detector and notebook
...
def main():
    ...
    for wav_path in wav_files:
        ...
        # Sanity check shape
        if spec.shape != (N_TIME_FRAMES, N_FREQ_BINS):
            raise ValueError(f"Unexpected shape {spec.shape} for {wav_path.name}")
    ...
    # Optionally stack all into a single array for later use
    stacked = np.stack(all_specs, axis=0)  # (n_clips, 15000, 484)
    out_stack = out_dir / "all_clips.npz"
    np.savez_compressed(out_stack, matrices=stacked)
    print(f"Saved stacked array to {out_stack.name} with shape {stacked.shape}")
    print("Done.")
