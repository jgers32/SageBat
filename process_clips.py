...
def wav_to_fixed_spectrogram_matrix(wav, sr,
    ...
    wav: 1D numpy array, float32 in [-1,1]
    sr: sample rate (int)
    """
...
def save_spectrogram_image(spec, freqs, times, out_png):
    """
    Save a visual spectrogram image.
    spec: shape (n_time_frames, n_freq_bins)
    """
    plt.figure(figsize=(10, 4))
    # Use imshow with extent as tuple of floats
...
def main():
    ...
    for wav_path in wav_files:
        ...
        # Time axis for visualization (center of each frame)
        duration = TARGET_CLIP_SEC
